...
async def handle_message(message: Message):
    ...
    try:
        ...
        if result.get("interrupt"):
            ...
            # ВАЖНО: Не используем Markdown для значений параметров, чтобы избежать ошибок парсинга
            # Telegram может неправильно парсить Markdown, если в значениях есть специальные символы
            
            # Маппинг английских названий параметров на русские
            param_translations = {
                "client_name": "Клиент",
                "card_type": "Тип карты",
                "amount": "Сумма",
                "rate": "Ставка",
                "term_months": "Срок",
                "calculation_type": "Тип начисления",
                "capitalization_months": "Период капитализации",
            ...
            # Перевод названия инструмента
            tool_translations = {
                "open_credit_card": "Открытие карты",
                "open_deposit": "Открытие вклада",
                "deposit_income_calculator": "Расчет дохода по вкладу",
                "currency_converter": "Конвертация валют",
                "search_products": "Поиск продуктов"
            }
            
            tool_name_ru = tool_translations.get(tool_name, tool_name)
            
            interrupt_message = (
                "⚠️ Требуется подтверждение операции\n\n"
                f"🔧 Инструмент: {tool_name_ru}\n"
                f"📋 Параметры:\n"
            )
            
            for key, value in tool_args.items():
                # Получаем русское название параметра
                param_name_ru = param_translations.get(key, key)
                
                # Форматируем значения для читаемости
                if isinstance(value, (int, float)):
                    ...
                    # Переводим некоторые значения
                    value_translations = {
                        "debit": "Дебетовая",
                        "credit": "Кредитная",
                        "simple": "Простой процент",
                        "compound": "С капитализацией",
                        "USD": "Доллар США",
                        "EUR": "Евро",
                        "RUB": "Рубль"
                    }
                    formatted_value = value_translations.get(value, value)
                else:
                ...
                if key == "rate":
                    ...
                    formatted_value = f"{formatted_value} мес."
                elif key == "amount" and not formatted_value.endswith("₽"):
                    formatted_value = f"{formatted_value}₽"
                
                interrupt_message += f"   • {param_name_ru}: {formatted_value}\n"
            
            interrupt_message += "\n❓ Выполнить операцию?"
            
            # Создаем inline кнопки
            # Используем простые кнопки "Да"/"Нет" для удобства пользователя
...
async def handle_hitl_callback(callback: CallbackQuery):
    ...
    try:
        ...
        logger.info(f"🔄 HITL callback received: {action} for chat {chat_id}")
        
        # Проверяем что interrupt существует
        if chat_id not in pending_interrupts:
            logger.warning(f"⚠️ Interrupt not found for chat {chat_id}")
        ...
        if action == "hitl_approve":
            decision = "approve"
            status_text = "✅ Операция подтверждена"
        else:  # hitl_reject
            decision = "reject"
            status_text = "❌ Операция отклонена"
        
        # Обновляем сообщение с статусом
        try:
            # Берем оригинальный текст сообщения
        ...
        # Удаляем из pending перед вызовом agent_resume
        interrupt_obj = pending_interrupts.pop(chat_id)
        logger.info(f"📋 Interrupt removed from pending, resuming agent with decision: {decision}")
        ...
        try:
            ...
            logger.info(f"📊 Agent resume completed, result keys: {result.keys()}")
            ...
            # Проверяем результат
            if not result:
                logger.error(f"❌ Empty result from agent_resume for chat {chat_id}")
                await callback.message.answer("❌ Произошла ошибка при обработке операции.")
            ...
            if not final_response:
                logger.warning(f"⚠️ Empty answer in result for chat {chat_id}")
                final_response = "Операция выполнена, но ответ не получен."
            
            # Добавляем источники если нужно
            ...
            await callback.answer()
            logger.info(f"✓ HITL {decision} processed successfully for chat {chat_id}")
            
        except Exception as e:
            logger.error(f"❌ Error in agent_resume for chat {chat_id}: {e}", exc_info=True)
            try:
                await processing_msg.delete()
            except:
                pass
            await callback.message.answer(
                f"❌ Произошла ошибка при выполнении операции: {str(e)[:200]}"
            )
            await callback.answer("❌ Ошибка при выполнении операции", show_alert=True)
