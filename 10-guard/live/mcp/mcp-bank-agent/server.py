...
# Mock номер договора для демонстрации (константа)
MOCK_CONTRACT_NUMBER = "ДГ-2025-1234567"
...
async def open_deposit(
    ...
    calculation_type: Annotated[
        Literal["simple", "compound"],
        Field(
            description="Тип начисления процентов: simple (простой) или compound (с капитализацией)",
    ...
    Returns:
        Форматированная информация о новом вкладе
    """
    logger.info(f"🔐 open_deposit called: client={client_name}, amount={amount}, "
                f"rate={rate}, term={term_months}, type={calculation_type}")
    
    # Используем константу для номера договора
    contract_number = MOCK_CONTRACT_NUMBER
    
    # Расчет дохода по вкладу
    if calculation_type == "simple":
        income, total = calculate_simple_interest(amount, rate, term_months)
    else:  # compound
        # Для compound используем ежемесячную капитализацию по умолчанию
        income, total, _ = calculate_compound_interest(amount, rate, term_months, 1)
    
    # Генерируем дату окончания вклада
    from datetime import datetime, timedelta
    end_date = (datetime.now() + timedelta(days=term_months * 30)).strftime("%d.%m.%Y")
    
    # Форматируем тип начисления для вывода
    calculation_type_ru = "простой процент" if calculation_type == "simple" else "с капитализацией"
    ...
    result = (
        ...
        f"   Тип начисления: {calculation_type_ru}\n"
