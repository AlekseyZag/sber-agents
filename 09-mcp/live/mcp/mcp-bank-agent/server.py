...
async def calculate_deposit_profit(
    ...
    - Эффективную ставку
    """
    logger.info(f"calculate_deposit_profit called: amount={initial_amount}, rate={annual_rate}%, "
                f"term={term_months} months, capitalization={capitalization}")
    
    try:
        # Конвертируем срок в годы для расчетов
        term_years = term_months / 12.0
        
        if capitalization:
            # Формула сложных процентов с ежемесячной капитализацией
            # Итоговая сумма = начальная * (1 + годовая_ставка/100/12)^(срок_в_месяцах)
            monthly_rate = annual_rate / 100.0 / 12.0
            final_amount = initial_amount * ((1 + monthly_rate) ** term_months)
        else:
            # Формула простых процентов
            # Итоговая сумма = начальная * (1 + годовая_ставка/100 * срок_в_годах)
            final_amount = initial_amount * (1 + (annual_rate / 100.0) * term_years)
        
        # Доход = итоговая сумма - начальная сумма
        profit = final_amount - initial_amount
        
        # Эффективная годовая ставка
        # Эффективная ставка = ((итоговая/начальная)^(1/срок_в_годах) - 1) * 100
        if term_years > 0:
            effective_rate = (((final_amount / initial_amount) ** (1.0 / term_years)) - 1.0) * 100.0
        else:
        ...
        result += f"⏱️  Срок: {term_months} месяцев ({term_years:.2f} лет)\n"
        result += f"🔄 Капитализация: {'Да (ежемесячная)' if capitalization else 'Нет'}\n\n"
        result += f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        result += f"💵 Итоговая сумма: {final_amount:,.2f} RUB\n"
        result += f"💸 Полученный доход: {profit:,.2f} RUB\n"
        result += f"📊 Эффективная ставка: {effective_rate:.2f}% годовых\n"
        
        if capitalization:
            result += f"\n💡 При капитализации эффективная ставка выше номинальной "
            result += f"({effective_rate:.2f}% vs {annual_rate:.2f}%)"
        
        return result
        
    except Exception as e:
        logger.error(f"Error calculating deposit profit: {e}", exc_info=True)
