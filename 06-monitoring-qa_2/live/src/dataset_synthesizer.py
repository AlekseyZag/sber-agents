...
def synthesize_qa_pairs_from_pdf(chunks: List, llm_model: str = "accounts/fireworks/models/gpt-oss-120b") -> List[Dict[str, Any]]:
    """
    ...
    """
    if not chunks:
        logger.warning("No chunks provided for PDF synthesis")
        return []
    
    logger.info(f"Starting synthesis from {len(chunks)} PDF chunks using model {llm_model}")
    
    # Проверка API ключа
    if not config.OPENAI_API_KEY:
        logger.error("OPENAI_API_KEY not set! Cannot synthesize Q&A pairs from PDF.")
        logger.error("Set OPENAI_API_KEY in .env file to enable PDF synthesis.")
        return []
...
{{
  "qa_pairs": [
    ...
    skipped_short = 0
    errors_count = 0
    error_details = []  # Собираем детали ошибок
    ...
    for i, chunk in enumerate(chunks):
        if len(chunk.page_content.strip()) < 100:
            skipped_short += 1
            logger.debug(f"Chunk {i} too short ({len(chunk.page_content.strip())} chars), skipping")
            continue
        
        content = None
        try:
            logger.info(f"Processing chunk {i+1}/{len(chunks)} (length: {len(chunk.page_content)} chars)")
            response = llm.invoke(
            ...
            for qa in data.get("qa_pairs", []):
                if "question" in qa and "answer" in qa:
                    ...
                    logger.info(f"✓ Successfully created Q&A pair from chunk {i+1}")
            ...
            errors_count += 1
            error_msg = f"Chunk {i+1}: JSON parsing error - {str(e)}"
            error_details.append(error_msg)
            logger.error(f"❌ {error_msg}")
            if content:
                logger.error(f"   LLM response (first 500 chars): {content[:500]}")
            else:
                logger.error(f"   No response content available")
            continue
        except Exception as e:
            errors_count += 1
            error_type = type(e).__name__
            error_msg = f"Chunk {i+1}: {error_type} - {str(e)}"
            error_details.append(error_msg)
            logger.error(f"❌ {error_msg}")
            logger.error(f"   Full error details:", exc_info=True)
            continue
    
    logger.info(f"=== PDF Synthesis Summary ===")
    ...
    logger.info(f"Successfully synthesized: {len(qa_pairs)} Q&A pairs")
    ...
    if len(qa_pairs) == 0 and len(chunks) > 0:
        logger.warning("⚠️ No Q&A pairs were synthesized from PDF!")
        logger.warning("Possible reasons:")
        if skipped_short == len(chunks):
            logger.warning("  - All chunks were too short (< 100 chars)")
        if errors_count > 0:
            logger.warning(f"  - {errors_count} errors occurred (see details above)")
            logger.warning("  - Check OPENAI_API_KEY and OPENAI_BASE_URL in .env")
            logger.warning("  - Check if LLM model is accessible and supports JSON output")
...
def create_dataset(data_dir: str, samples_per_file: int = 2) -> List[Dict[str, Any]]:
    ...
    logger.info(f"Loaded {len(pdf_chunks)} chunks from PDF files")
