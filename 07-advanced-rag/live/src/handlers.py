...
async def cmd_index_status(message: Message):
    ...
    if stats['retrieval_mode'] in ['hybrid', 'hybrid_reranker']:
        ...
        status_text += f"Chunks для BM25: {chunks_count}\n"
    
    status_text += f"\n🔍 *Retrieval: {stats['retrieval_mode']}*\n"
    ...
    # Информация о LLM
    status_text += f"\n🤖 *LLM: {stats.get('llm_provider', 'N/A')}*\n"
    if stats.get('llm_provider') == 'openai':
        status_text += f"• Модель: {stats.get('llm_model', 'N/A')}\n"
        status_text += f"• Query transform: {stats.get('llm_query_transform_model', 'N/A')}\n"
    elif stats.get('llm_provider') == 'ollama':
        status_text += f"• Модель: {stats.get('llm_model', 'N/A')}\n"
        status_text += f"• Query transform: {stats.get('llm_query_transform_model', 'N/A')}\n"
        status_text += f"• Base URL: {stats.get('ollama_base_url', 'N/A')}\n"
